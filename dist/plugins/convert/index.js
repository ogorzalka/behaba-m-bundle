...
function convertFile(file, ext, args, files, results) {
  var nameData = { '%e': ext };
  if (minimatch(file, args.src, { matchBase: true })) {
    ...
    ['density', 'blur', 'rotate', 'flip', 'strip', 'quality'].forEach(function (setting) {
    ...
    newName = path.join(path.dirname(files[file].link), newName);
    console.log(newName);
    ...
    /*, newName, convertArgs.quality, function() {*/
